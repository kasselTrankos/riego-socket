...
// irrigate => f => new CronJob(date.of(irrigate.date).value, ()=> f(irrigate));
// const kronJob = job => IO.of(kron(job));
...
const kron =  date => {
  ...
  return new Task((_, resolve)=> {
    console.log(date, '11111', new Date());
...
const throwDates = dates => {
  ...
  if(t) {
    kron(getDate(t)).fork(console.log, (job)=> {
      const irrigate  = dates.next();
      if(time) {
        irrigate(list.previous);
        job.setTime(new CronTime(getDate(irrigate)));
        job.start();
      } else {
        job.stop();
      }
