...
const iterator = function _list(elms) {
  let items = elms;
  let last = 0;
  return {
    previous: last,
...
function scheduler () {
  ...
  const decode = task => task.map(compose(prop('dates'), JSON.parse, toString));
  const filterOverNow = linkedList => linkedList.filter(isOverNow);
  const sortAsc = linkedList => linkedList.sort();
  const toArray = linkedList => linkedList.toArray();
  const getIterator = compose(iterator, toArray, sortAsc, filterOverNow, Irrigation.from)
