...
const throwDateCron =  dates => index => {
  ...
  job = new CronJob(date, () => {
    ...
    console.log(`${new Date()} (${process.pid})this irrigate at ${dates[index].date} is done`);
    const next = ++index;
    gotNextKal(next)
    ? kalendar(next)
    : notifyEnd();
...
function sheduler () {
  ...
  // console.log(dates[0], ' vamos a lo loco');
  console.log(`${new Date()} (${process.pid}) start scheduler/or resart if chage the json`);
  // job.stop();
